...
let indexGenerate = document.getElementById('amount')
let generateBtn = document.querySelector('button')
let generatedTxt = document.getElementById('result')

function generateTxt(event){

event.preventDefault()
generatedTxt.innerHTML = ''
for(let i = 0; i < indexGenerate.value; i++){
  let randomTxt = Math.random() * dummyText.length

  randomTxt = Math.floor(randomTxt)

  generatedTxt.append(dummyText[randomTxt])
}
  if(indexGenerate.value < 0){
    alert('can not generate NEGETIVE value , \n please insert a correct value')
  }
  
}


generateBtn.addEventListener('click', generateTxt)